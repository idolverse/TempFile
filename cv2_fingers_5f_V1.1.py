...
def serial_monitor(ser, status_signal):
    ...
    while True:
        try:
            ...
            status_signal.emit("串口连接异常")
            break
        time.sleep(0.01)

def draw_text_with_chinese(frame, text, position, font_size=20, color=(255, 255, 0)):
    """使用PIL绘制中文文本"""
    try:
...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        # 获取屏幕分辨率以调整摄像头捕获尺寸
        screen_res = QApplication.desktop().availableGeometry()
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, screen_res.width())
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, screen_res.height())
        ...
        while self.running:
            ...
            if prevTime != 0:
                ...
                # 使用PIL绘制中文
                frame = draw_text_with_chinese(frame, f"实际FPS: {int(fps)}", (10, 70), 24, (255, 0, 255))
            prevTime = currentTime
            
            # 显示处理参数
            frame = draw_text_with_chinese(frame, f"滑动窗口: {self.WINDOW_SIZE}帧", (10, 110), 24, (255, 255, 0))
            frame = draw_text_with_chinese(frame, f"帧计数: {self.frame_count}", (10, 150), 24, (255, 255, 0))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(0, 0, self.screen_width, self.screen_height)  # 从屏幕左上角开始，尺寸为屏幕大小
        self.setWindowState(Qt.WindowFullScreen)  # 最大化窗口（可选，根据需求决定是否使用全屏模式）
    ...
    def init_ui(self):
        ...
        main_layout.addWidget(video_frame, 6)  # 视频区占60%
        ...
        title_label.setStyleSheet("font-size: 16pt; font-weight: bold; margin: 10px 0;")
        ...
        self.start_btn.setMinimumSize(160, 60)
        ...
        self.stop_btn.setMinimumSize(160, 60)
        ...
        self.stop_btn.setEnabled(False)  # 初始时禁用结束按钮
        button_layout.addWidget(self.stop_btn)
        
        # 退出按钮
        self.exit_btn = QPushButton("退出")
        self.exit_btn.setMinimumSize(160, 40)
        ...
        status_label.setStyleSheet("font-size: 12pt; font-weight: bold; margin: 15px 0 8px;")
        control_layout.addWidget(status_label)
        
        self.status_text = QLabel("系统未启动")
        self.status_text.setWordWrap(True)
        self.status_text.setStyleSheet("background-color: #f0f0f0; padding: 8px; border-radius: 5px; font-size: 11pt;")
        control_layout.addWidget(self.status_text)
        
        main_layout.addWidget(control_frame, 4)  # 控制区占40%
    ...
    def start_program(self):
        ...
        try:
            ...
            self.video_thread.update_status.connect(self.status_text.setText)
